...
//   orbitAMatrix         (6,6),
   ...
   fullAMatrix          (6,6),
   fullSTMRowCount      (6),
   srpShapeModel        ("Spherical"),    // Its value should be "Spherical" or"SPADFile"     // made changes by TUAN NGUYEN
   spadSRPFile          (""),
   spadSrpFileFullPath  (""),
...
//   hardwareList      = a.hardwareList;
   ...
   {
      Cd0 = coeffDrag / (1 + crEpsilon);                   // Cd0 = Cd / (1+Cd_epsilon)
   ...
   {                                                                                               // made changes by TUAN NGUYEN
      Cd0 = spadDragScaleFactor / (1 + crEpsilon);        // Cd0 = Cd / (1+Cd_epsilon)             // made changes by TUAN NGUYEN
...
//   ephemMgr           = NULL;
   ...
   if (dragShapeModel == "Spherical")                                                               // made changes by TUAN NGUYEN
   {                                                                                                // made changes by TUAN NGUYEN
      Cd0 = coeffDrag / (1 + cdEpsilon);                   // Cd0 = Cd/(1+Cd_epsilon)
      Cd_EpsilonSigma = coeffDragSigma / Cd0;              // stdiv[Cd_epsilon] = stdiv[Cd]/Cd0
   }                                                                                                // made changes by TUAN NGUYEN
   else if (dragShapeModel == "SPADFile")                                                           // made changes by TUAN NGUYEN
   {                                                                                                // made changes by TUAN NGUYEN
      Cd0 = spadDragScaleFactor / (1 + cdEpsilon);         // Cd0 = Cd/(1+Cd_epsilon)               // made changes by TUAN NGUYEN
      Cd_EpsilonSigma = spadDragScaleFactorSigma / Cd0;    // stdiv[Cd_epsilon] = stdiv[Cd]/Cd0     // made changes by TUAN NGUYEN
   }                                                                                                // made changes by TUAN NGUYEN
...
Real Spacecraft::SetRealParameter(const Integer id, const Real value)
{
   ...
   {
      //parmsChanged = true;                                   // made changes by TUAN NGUYEN
      //return SetRealParameter("Cd",value);                   // made changes by TUAN NGUYEN
   ...
   {
      //parmsChanged = true;                                   // made changes by TUAN NGUYEN
      //return SetRealParameter("Cr",value);                   // made changes by TUAN NGUYEN

      //if (((value >= 0.0) && (value <= 2.0)) || (constrainCr == false))    // made changes by TUAN NGUYEN
   ...
   if (id == SPAD_DRAG_SCALE_FACTOR_NOISESIGMA)
   {
      ...
      {                                                                                     // made changes by TUAN NGUYEN
         ...
         Real spadDragScaleFactor0 = spadDragScaleFactor / (1 + cdEpsilon);           // Cd0 = Cd/(1+Cd_epsilon)
   ...
   if (id == SPAD_SRP_SCALE_FACTOR_NOISESIGMA)
   {
      ...
      {                                                                                             // made changes by TUAN NGUYEN
         ...
         Real spadSRPScaleFactor0 = spadSRPScaleFactor / (1 + crEpsilon);              // Cr0 = Cr/(1+Cr_epsilon)
...
Integer Spacecraft::GetStmRowId(const Integer forRow)
{
   ...
   if ((forRow - stmIndices.size() < externalStmIndices.size()) && (forRow >= 0))
 ...
 *
 * This method is used to add rows to teh stm that come from an external source.
 * It is used to increase teh size of the Spacecraft STM for new solve-for
                     ...
                     GmatBase *stmObj, const Integer parmIndex)
{
   ...
   else if (std::find(externalStmEntries.begin(), externalStmEntries.end(),
         stmLabel) == externalStmEntries.end())
   {
      externalStmEntries.push_back(stmLabel);
      externalStmSources.push_back(stmObj);

      // 21 is a magic number here, and needs to be replaced
      Integer index = 21 + externalStmEntries.size();
...
bool Spacecraft::SetSRPShapeModel(std::string model)
{
   ...
   if (model == "Spherical")
   {
      // throw an error when using SPADSRPScaleFactor as a solve-for variable
      for (Integer i = 0; i < solveforNames.size(); ++i)
      {
         if (solveforNames[i] == "SPADSRPScaleFactor")
         {
            throw SpaceObjectException("Error: " + GetName() +
               ".SPADSRPScaleFactor parameter is not allowed to be a solve-for variable due to SRP Model is Spherical.\n");
         }
      }
   }
   else if (model == "SPADFile")
   {
      // throw an error when using Cr as a solve-for variable
      for (Integer i = 0; i < solveforNames.size(); ++i)
      {
         if (solveforNames[i] == "Cr")
         {
            throw SpaceObjectException("Error: " + GetName() +
               ".Cr parameter is not allowed to be a solve-for variable due to SRP Model is SPADFile.\n");
         }
      }
   }
...
bool Spacecraft::SetDragShapeModel(std::string model)
{
   if (model == "Spherical")
   {
      ...
      for (Integer i = 0; i < solveforNames.size(); ++i)
      {
         ...
         {
            throw SpaceObjectException("Error: " + GetName() +
               ".SPADDragScaleFactor parameter is not allowed to be a solve-for variable due to Drag Model is Spherical.\n");
         }
      }
   }
   else if (model == "SPADFile")
   {
      ...
      for (Integer i = 0; i < solveforNames.size(); ++i)
      {
         ...
         {
            throw SpaceObjectException("Error: " + GetName() +
               ".Cd parameter is not allowed to be a solve-for variable due to Drag Model is SPADFile.\n");
         }
      }
   }
