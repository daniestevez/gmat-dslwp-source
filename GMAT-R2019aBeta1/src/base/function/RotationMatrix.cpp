...
bool RotationMatrix::Execute(ObjectInitializer *objInit, bool reinitialize)
{
   ...
   // Check for output info, there shoud be 2 output
   ...
   if (input1_coordSys == NULL)
   {
      throw FunctionException
         ("RotationMatrix::Execute()  input1_coordsys remains unset\n");
   }
   if (input2_epoch == NULL)
   {
      throw FunctionException
         ("RotationMatrix::Execute()  input2_epoch remains unset\n");
   }
   if (input3_epochFormat == NULL)
   {
      throw FunctionException
         ("RotationMatrix::Execute()  input3_epochFormat remains unset\n");
   }
...
//   std::string axesType = inCoordSys->GetStringParameter("axesType");
   ...
   Rmatrix33 lastRotation = cloneCoordSys->GetLastRotationMatrix();
   Rmatrix33 lastRotationDot = cloneCoordSys->GetLastRotationDotMatrix();
