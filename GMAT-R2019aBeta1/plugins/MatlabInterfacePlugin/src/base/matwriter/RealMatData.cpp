      ...
      mxArray *mat_struct, mwIndex index)
{
   ...
   if (numDim == 2U)
   {
      if (isJagged)
      {
         ...
         for (int i = 0; i < n_size; i++)
         {
            ...
            mxArray *pa2;
            pa2 = mxCreateNumericMatrix(0, 0, mxDOUBLE_CLASS, mxREAL);

            mwSize m_size = realData[i].size();

            // set data
            data_tmp = (double *)mxMalloc(m_size* sizeof(double));

            for (int j = 0; j < m_size; j++)
            {
               data_tmp[j] = realData[i][j];
            }

            // write the data
            mxSetData(pa2, data_tmp);
            mxSetM(pa2, m_size);
            mxSetN(pa2, 1);
      ...
      else
      {
         ...
         for (int i = 0; i < m_size; i++)
         {
            ...
            {
               data_tmp[i + m_size*j] = realData[i][j];
            }
         }
   ...
   else if (numDim == 3U)
   {
      ...
      for (int i = 0; i < idx1_size; i++)
      {
         ...
         mwSize idx3_size = realData3D[i][0].size();

         // set data
         data_tmp = (double *)mxMalloc(idx2_size*idx3_size* sizeof(double));

         for (int j = 0; j < idx2_size; j++)
         {
            for (int k = 0; k < idx3_size; k++)
            {
               data_tmp[j + idx2_size*k] = realData3D[i][j][k];
            }
         }

         // write the data
         mxSetData(pa2, data_tmp);
         mxSetM(pa2, idx2_size);
         mxSetN(pa2, idx3_size);
