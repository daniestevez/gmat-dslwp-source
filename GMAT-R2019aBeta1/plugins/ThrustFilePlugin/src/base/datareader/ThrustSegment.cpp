...
Real ThrustSegment::SetRealParameter(const Integer id, const Real value)
{
   ...
   if (id == TSF_SIGMA)
   {
      if (value > 0.0)
         tsfSigma = value;
      // Should we throw ODEModelException if value <= 0.0?
      // Note Spacecraft::SetRealParameter for CD_SIGMA_ID also updates a covariance matrix
      return tsfSigma;
   }
      ...
      const Integer index)
{
   ...
   {
      #ifdef DEBUG_TSF_SOLVEFOR
         MessageInterface::ShowMessage("ThrustSegment::SetStringParameter 2\n");
         for (UnsignedInt i = 0; i < solveFors.size(); ++i)
            MessageInterface::ShowMessage("ThrustSegment::SetStringParameter "
                  "Solve For  %s\n", solveFors[i].c_str());
      #endif
