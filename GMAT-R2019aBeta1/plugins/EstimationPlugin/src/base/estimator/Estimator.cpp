...
//------------------------------------------------------------------------------
// bool AddMatData(DataBucket &matData, bool useStatus, Integer index)
//------------------------------------------------------------------------------
/**
 * Method used to add this iteration's data to the MATLAB .mat file
 *
 * @param matData    The DataBucket containing the data to add
 * @param useStatus  Flag to indicate if the elementStatus should be added to
 *                   the structure
 ...
 */
//------------------------------------------------------------------------------
bool Estimator::AddMatData(DataBucket &data, bool useStatus, Integer index)
{
   bool retval = true;
   ...
   if (useStatus)
   {
      writerData = matWriter->GetContainer(Gmat::REAL_TYPE, "Status");
      std::vector<RealArray> statData;
      statData.push_back(data.elementStatus);
...
bool Estimator::WriteMatData()
{
   ...
   dataDesc = GetMatDataDescription(matData, true);
   matWriter->DescribeData(dataDesc, iterationsTaken);
   matWriter->WriteData("Iteration");


   // Add observation data
   matWriter->ClearData();

   AddMatData(matObsData, false);

   dataDesc = GetMatDataDescription(matObsData, false);
   matWriter->DescribeData(dataDesc);
   matWriter->WriteData("Observations");

   // Add estimation config data
   matWriter->ClearData();

   AddMatData(matConfigData, false);

   dataDesc = GetMatDataDescription(matConfigData, false);
   matWriter->DescribeData(dataDesc);
...
//------------------------------------------------------------------------------
// StringArray GetMatDataDescription(DataBucket &data, bool useStatus)
//------------------------------------------------------------------------------
/**
 ...
 */
//------------------------------------------------------------------------------
StringArray Estimator::GetMatDataDescription(DataBucket &data, bool useStatus)
{
   StringArray dataDesc;
...
void Estimator::AddMatlabData(const MeasurementInfoType &measStat)
{
   if (writeMatFile && (matWriter != NULL))
   {
      ...
      if (matIndex.count("Calc") == 0)
      {
         matIndex["Calc"] = matData.AddRealContainer("Calculated", 0);
         matIndex["Omc"] = matData.AddRealContainer("ObsMinusCalc", 0);
         matIndex["Elevation"] = matData.AddRealContainer("Elevation", 1);
         matIndex["ObsEditFlag"] = matData.AddStringContainer("ObsEditFlag", 1);
      ...
      if (measStat.modelSize > 0)
      {
         ...
         if (measStat.isCalculated)
         {
            matData.realValues[matIndex["Calc"]][matMeasIndex] = measStat.measValue;
            matData.realValues[matIndex["Omc"]][matMeasIndex] = measStat.residual;
      ...
      matData.stringValues[matIndex["ObsEditFlag"]][matMeasIndex][0] = measStat.removedReason;
      ...
      if (iterationsTaken == 0)
      {
         if (matObsIndex.count("EpochTAI") == 0)
         {
            ...
            matObsIndex["Obs"] = matObsData.AddRealContainer("Observed", 0);
            matObsIndex["Weight"] = matObsData.AddRealContainer("Weight", 0);
            matObsIndex["Part"] = matObsData.AddStringContainer("Participants", 0);
            matObsIndex["Type"] = matObsData.AddStringContainer("Type", 1);

            matObsIndex["Frequency"] = matObsData.AddRealContainer("Frequency", 1);
            matObsIndex["FreqBand"] = matObsData.AddRealContainer("FrequencyBand", 1);
         ...
         matObsData.realValues[matObsIndex["Obs"]][matMeasIndex] = currentObs->value;
         ...
         if (currentObs->typeName == "DSN_SeqRange")
         {
            matObsData.realValues[matObsIndex["FreqBand"]][matMeasIndex][0] = currentObs->uplinkBand;
...
void Estimator::AddMatlabIterationData()
{
   if (writeMatFile && (matWriter != NULL))
   {
      ...
      for (UnsignedInt i = 0; i < stateMap->size(); ++i)
      {
         ...
         {
            finalCovariance(i, j) *= conversionValue;
            finalCovariance(j, i) *= conversionValue;
         }
      }
      ...
      for (UnsignedInt i = 0; i < stateMap->size(); ++i)
      {
         ...
         for (UnsignedInt j = 0; j < stateMap->size(); ++j)
         {
            finalKeplerCovariance(i, j) *= conversionValue;
            finalKeplerCovariance(j, i) *= conversionValue;
         }
      }
...
std::string Estimator::GetElementUnit(ListItem* infor, std::string stateType) const
{
   ...
   else if (infor->elementName == "Bias")
   {
      if (infor->object->IsOfType(Gmat::MEASUREMENT_MODEL))
      {
	    ...
	    unit = "km";
	 else if (mm->IsOfType("DeltaRangeRate"))
	    unit = "km/s";
         else
            unit = "****";
      }
...
void Estimator::WriteReportFileHeaderPart2()
{
   ...
   // 5. Get row index for SPADSRPScaleFactorSigma and SPADDargScaleFactorSigma
...
void Estimator::WriteReportFileHeaderPart2b()
{
   ...
   {
      Real conversionValue = GetEpsilonConversion(i);
      for (Integer j = 0; j < aprioriCov.GetNumRows(); ++j)
      {
         aprioriCov(j, i) *= conversionValue;
         aprioriCov(i, j) *= conversionValue;
      }
   }
...
void Estimator::WriteReportFileSummaryPart0(Solver::SolverState sState)
{
   if (sState == ESTIMATING)
   {
      if (writeMeasurmentsAtEnd)
      {
         for (UnsignedInt ii = 0; ii < measStats.size(); ii++)
         {
            MeasurementInfoType measStat = measStats[ii];

            if ((measStat.recNum != 0) && (GmatMathUtil::Mod(measStat.recNum, 80) < 0.001))
               WritePageHeader();
      ...
      // Add iteration related data to .mat file
...
void Estimator::WriteReportFileSummaryPart3(Solver::SolverState sState)
{
   ...
   if (sState == ESTIMATING)
   {
      ...
      for (UnsignedInt i = 0; i < map->size(); ++i)
      {
         ...
         {
            covar(i, j) *= conversionValue;
            covar(j, i) *= conversionValue;
         }
      }
