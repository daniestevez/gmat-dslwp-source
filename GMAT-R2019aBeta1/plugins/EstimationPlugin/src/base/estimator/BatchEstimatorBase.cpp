 ...
 */
//------------------------------------------------------------------------------
const StringArray& BatchEstimatorBase::GetPropertyEnumStrings(const Integer id) const
{
...
Solver::SolverState BatchEstimatorBase::AdvanceState()
{
   ...
   try
   {
      ...
      case ESTIMATING:
         ...
         // Add .mat data
         if (matWriter != NULL)
            if (!AddMatData(matData, true, iterationsTaken))
...
void BatchEstimatorBase::CheckCompletion()
{
   ...
   else
   {
      ...
      esm.ResetDragCoefficient();                        // made changes by TUAN NGUYEN
