...
bool EstimationStateManager::SetProperty(GmatBase *obj)
{
   ...
   {
#ifdef DEBUG_STATE_CONSTRUCTION
	   MessageInterface::ShowMessage("EstimationStateManager::SetProperty   %s%s \n", obj->GetFullName() ,solforNames[i].c_str());
#endif
...
//   else if (obj->IsOfType("ThrustSegment"))                         // Thrust Scale Factor Solve For
   ...
   for (UnsignedInt i = 0; i < solveforList.size(); ++i)
   {
      ...
      for (UnsignedInt i = 0; i < solveforList.size(); ++i)
         MessageInterface::ShowMessage("EstimationStateManager::GetSolveForList   %s\n", solveforList[i].c_str());
   #endif
...
//--------------------------------------------------------------------------------
/**
* This function is used to reset Cr_Epsilon avd Cd_Epsilon value to 0 when an 
* estimation iteration is rerun. It set:
*               Cr0 to Cr0 * (1 + CrEpsilon) and CrEpsilon to 0
*               Cd0 to Cd0 * (1 + CdEpsilon) and CdEpsilon to 0
* This function is called when starting a new iteration.
*/
//--------------------------------------------------------------------------------

void EstimationStateManager::ResetDragCoefficient()
{
#ifdef DEBUG_OBJECT_UPDATES
   MessageInterface::ShowMessage("Reset drag force coefficients to objects\n");
#endif
   ...
   for (Integer index = 0; index < stateSize; ++index)
   {
      ...
      if (stateMap[index]->elementName == "Cr_Epsilon")
      {
         Spacecraft* sc = (Spacecraft*)(stateMap[index]->object);
         std::string srpShapeModel = sc->GetSRPShapeModel();
         if (srpShapeModel == "Spherical")
         {
            // Get value of Cr = Cr0 * (1 + Cr_Epsilon)
            Real cr = sc->GetRealParameter("Cr");
            // Set value of Cr and reset Cr_Epsilon to 0
            sc->SetRealParameter("Cr", cr);
         }
         else if (srpShapeModel == "SPADFile")
         {
            Real srpScaleFactor = sc->GetRealParameter("SPADSRPScaleFactor");
            sc->SetRealParameter("SPADSRPScaleFactor", srpScaleFactor);
         }
         else
            throw EstimatorException("Error: Spacecraft '" + sc->GetName() + "' SRP shape model was not specified. Its value should be Spherical or SPADFile.\n");
      }
      else if (stateMap[index]->elementName == "Cd_Epsilon")
      {
         Spacecraft* sc = (Spacecraft*)(stateMap[index]->object);
         std::string dragShapeModel = sc->GetDragShapeModel();
         if (dragShapeModel == "Spherical")
         {
            // Get value of Cd = Cd0 * (1 + Cd_Epsilon)
            Real cd = sc->GetRealParameter("Cd");
            // Set value of Cd and reset Cd_Epsilon to 0
            sc->SetRealParameter("Cd", cd);
         }
         else if (dragShapeModel == "SPADFile")
         {
            Real dragScaleFactor = sc->GetRealParameter("SPADDragScaleFactor");
            sc->SetRealParameter("SPADDragScaleFactor", dragScaleFactor);
         }
         else
            throw EstimatorException("Error: Spacecraft '" + sc->GetName() + "' DragForce shape model was not specified. Its value should be Spherical or SPADFile.\n");
      }
...
//         elementLength = stateMap[h]->length;
         ...
         if (hasDstm)
         {
            ...
            for (Integer i = 0; i < stmSize; ++i)
            {
               ...
               for (Integer j = 0; j < stmSize; ++j)
               {
                  MessageInterface::ShowMessage("%.12lf   ", (*dstm)(i, j));
               }
               MessageInterface::ShowMessage("]\n");
            }
            Integer index = 0;
