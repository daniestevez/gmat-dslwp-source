...
bool SignalBase::StepParticipant(Real stepToTake, bool forTransmitter)
{
   ...
   if (prop)      // Handle spacecraft
   {
      ...
      if (forTransmitter)
      {
         ...
         SpacePoint* forcemodelOrigin = theData.tPropagator->GetODEModel()->GetForceOrigin();        // the origin of the coordinate system used in forcemodel
         state = state + (forcemodelOrigin->GetMJ2000PrecState(tPrecTimeNew) - spacecraftOrigin->GetMJ2000PrecState(tPrecTimeNew));
      }
      else
      {
         ...
         SpacePoint* forcemodelOrigin = theData.rPropagator->GetODEModel()->GetForceOrigin();        // the origin of the coordinate system used in forcemodel
         state = state + (forcemodelOrigin->GetMJ2000PrecState(rPrecTimeNew) - spacecraftOrigin->GetMJ2000PrecState(rPrecTimeNew));
      }
