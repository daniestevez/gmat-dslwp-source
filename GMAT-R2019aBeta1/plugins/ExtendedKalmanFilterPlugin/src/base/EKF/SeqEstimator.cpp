...
void SeqEstimator::WriteHeader()
{
   ...
   taiMjdEpoch = theTimeConverter->Convert(currentEpochGT,
         TimeSystemConverter::A1MJD, TimeSystemConverter::TAIMJD);
   utcMjdEpoch = theTimeConverter->Convert(currentEpochGT,
         TimeSystemConverter::A1MJD, TimeSystemConverter::UTCMJD);
   textFile << "   " << taiMjdEpoch.ToString() << " TAI Mod. Julian\n";
   utcEpoch = theTimeConverter->ConvertMjdToGregorian(utcMjdEpoch.GetMjd());
...
void SeqEstimator::WriteSummary(Solver::SolverState sState)
{
   ...
   if (sState == ESTIMATING)
   {
      ...
      for (UnsignedInt i = 0; i < map->size(); ++i)
      {
         if ((*map)[i]->elementName == "Cr_Epsilon")
         {
            ...
            if (((Spacecraft*)(*map)[i]->object)->GetSRPShapeModel() == "Spherical")           // made changes by TUAN NGUYEN
               Cr0 = (*map)[i]->object->GetRealParameter("Cr") / (1 + (*map)[i]->object->GetRealParameter("Cr_Epsilon"));
            else if (((Spacecraft*)(*map)[i]->object)->GetSRPShapeModel() == "SPADFile")       // made changes by TUAN NGUYEN
               Cr0 = (*map)[i]->object->GetRealParameter("SPADSRPScaleFactor") / (1 + (*map)[i]->object->GetRealParameter("Cr_Epsilon"));  // made changes by TUAN NGUYEN
         ...
         else if ((*map)[i]->elementName == "Cd_Epsilon")
         {
            ...
            Real Cd0;                                                                       // made changes by TUAN NGUYEN
            if (((Spacecraft*)(*map)[i]->object)->GetDragShapeModel() == "Spherical")       // made changes by TUAN NGUYEN
               Cd0 = (*map)[i]->object->GetRealParameter("Cd") / (1 + (*map)[i]->object->GetRealParameter("Cd_Epsilon"));
            else if (((Spacecraft*)(*map)[i]->object)->GetDragShapeModel() == "SPADFile")   // made changes by TUAN NGUYEN
               Cd0 = (*map)[i]->object->GetRealParameter("SPADDragScaleFactor") / (1 + (*map)[i]->object->GetRealParameter("Cd_Epsilon"));   // made changes by TUAN NGUYEN
         ...
         else if (GmatStringUtil::EndsWith((*map)[i]->elementName, ".TSF_Epsilon"))     // <segmentName>.TSF_Epsilon
         {
             ...
             // Get ThrustSF0
             Real ThrustSF0 = (*map)[i]->object->GetRealParameter(tsfName) /
                 (1 + (*map)[i]->object->GetRealParameter((*map)[i]->elementName));

             // multiply row and column i with ThrustSF0
             for (int j = 0; j < covar.GetNumColumns(); ++j)
                 covar(i, j) *= ThrustSF0;
             for (int j = 0; j < covar.GetNumRows(); ++j)
                 covar(j, i) *= ThrustSF0;
      ...
      textFile << "Iteration " << iterationsTaken << ":  Statistics \n"
               ...
               << "   Records Used For Estimation : " << numRemovedRecords["N"] << "\n"
...
//            << bestResidualRMS << "\n\n";
   ...
   for (UnsignedInt i = 0; i < map->size(); ++i)
   {
      ...
      else if (GmatStringUtil::EndsWith((*map)[i]->elementName, ".TSF_Epsilon"))       // <segmentName>.TSF_Epsilon
      {
          ...
          // Get ThrustSF0
          Real ThrustSF0 = (*map)[i]->object->GetRealParameter(tsfName) / (1 + (*map)[i]->object->GetRealParameter((*map)[i]->elementName));

          // multiply row and column i with ThrustSF0
          for (int j = 0; j < finalCovariance.GetNumColumns(); ++j)
              finalCovariance(i, j) *= ThrustSF0;
          for (int j = 0; j < finalCovariance.GetNumRows(); ++j)
              finalCovariance(j, i) *= ThrustSF0;
      }
   }
   ...
   if (valid)
   {
      ...
      for (UnsignedInt i = 0; i < map->size(); ++i)
      {
         ...
         else if (GmatStringUtil::EndsWith((*map)[i]->elementName, ".TSF_Epsilon"))   // <segmentName>.TSF_Epsilon
         {
             ...
             // multiply row and column i with ThrustSF0
             for (int j = 0; j < finalKeplerCovariance.GetNumColumns(); ++j)
                 finalKeplerCovariance(i, j) *= ThrustSF0;
             for (int j = 0; j < finalKeplerCovariance.GetNumRows(); ++j)
                 finalKeplerCovariance(j, i) *= ThrustSF0;
         }
      }
